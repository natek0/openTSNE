...
# -*- coding: utf-8 -*-
"""
Benchmark on the 3-D Swiss-roll manifold.

Example (baselines only, colour + grid):
    python swissroll_bench.py --noise 0.05 --n_samples 2000 \
        --plot_dir figs_sw --skip_csr
"""
...
def _plot_scatter(points : np.ndarray,
    ...
    """Save a 3-D or 2-D scatter with optional colour gradient."""
    if points.shape[1] == 3:
...
def main(argv: list[str] | None = None) -> None:
    p = argparse.ArgumentParser(description="Swiss-roll NLDR benchmark")
    ...
    p.add_argument("--outfile",  default="swissroll_results.csv")
    p.add_argument("--plot_dir", default="figs_sw")
    ...
    # ------------------------------------------------------------------ #
    # 1) Generate raw data (X) and latent parameter t
    # ------------------------------------------------------------------ #
    X, t = datasets.make_swiss_roll(args.n_samples,
                                    noise=args.noise,
                                    random_state=rng)
    _plot_scatter(X, "Raw Swiss-roll",
